...
>
    ...
    <div class="w-full flex flex-row gap-4 mb-4">
        <div class="w-1/3">
            <h1 class="text-xl font-semibold">{{ $recordTypeLabel }}</h1>
            <p class="text-sm text-gray-600">
                {{ $project->name }} · Sample {{ $sample?->lab_sample_id ?: $sample?->external_id ?: '—' }}
                · {{ $performedAt ? \Illuminate\Support\Carbon::parse($performedAt)->format('Y-m-d') : '—' }}
        ...
        <table class="w-full text-sm bg-white shadow rounded">
            <thead class="bg-gray-50 text-left">
                <tr>
                    ...
                    <th class="px-4 py-3 whitespace-nowrap">Stdev</th>
                ...
                @forelse($compounds as $c)
                    ...
                    >
                        ...
                        >
                            ...
                            <td wire:key="value-field-{{ $c->id }}" class="px-4 py-3">
                                <div
                                    x-data="{
                                        editing: false,
                                        value: @js($c->value !== null ? number_format((float)$c->value, 4, '.', '') : ''),
                                        original: @js($c->value !== null ? number_format((float)$c->value, 4, '.', '') : ''),
                                        cancelled: false,
                                        startEdit() { this.editing = true; this.$nextTick(() => this.$refs.valueInput.select()); },
                                        cancel() { this.cancelled = true; this.value = this.original; this.editing = false; },
                                        save() {
                                            if (!this.cancelled && this.value !== this.original) {
                                                $wire.updateValue({{ $c->id }}, this.value);
                                                this.original = this.value;
                                            }
                                            this.cancelled = false;
                                            this.editing = false;
                                    ...
                                    class="flex items-center gap-1.5 min-w-0"
                                >
                                    <span x-show="!editing" @dblclick.stop="startEdit()" class="cursor-default" title="Double-click to edit" x-text="value || '—'"></span>
                                    <input
                                        x-show="editing" x-ref="valueInput" x-model="value" type="text" inputmode="decimal"
                                        @click.stop @keydown.enter.prevent="save()" @keydown.escape.prevent="cancel()" @blur="save()"
                                        class="border border-gray-300 rounded px-2 py-0.5 text-sm w-24 focus:outline-none focus:ring-2 focus:ring-black"
                                    />
                                    <button x-show="!editing" @click.stop="startEdit()" class="shrink-0 opacity-0 group-hover:opacity-40 hover:!opacity-100 transition-opacity" title="Edit value">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a2 2 0 01-1.414.586H8v-2.414a2 2 0 01.586-1.414z"/></svg>
                                    </button>
                                </div>
                            </td>

                            {{-- Unit (double-click to edit) --}}
                            <td wire:key="unit-field-{{ $c->id }}" class="px-4 py-3">
                                <div
                                    x-data="{
                                        editing: false,
                                        value: @js($c->unit ?? ''),
                                        original: @js($c->unit ?? ''),
                                        cancelled: false,
                                        startEdit() { this.editing = true; this.$nextTick(() => this.$refs.unitInput.select()); },
                                        cancel() { this.cancelled = true; this.value = this.original; this.editing = false; },
                                        save() {
                                            if (!this.cancelled && this.value !== this.original) {
                                                $wire.updateUnit({{ $c->id }}, this.value);
                                                this.original = this.value;
                                            }
                                            this.cancelled = false;
                                            this.editing = false;
                                ...
                                >
                                    <span x-show="!editing" @dblclick.stop="startEdit()" class="cursor-default" title="Double-click to edit" x-text="value || '—'"></span>
                                    <input
                                        x-show="editing" x-ref="unitInput" x-model="value" type="text"
                                        @click.stop @keydown.enter.prevent="save()" @keydown.escape.prevent="cancel()" @blur="save()"
                                        class="border border-gray-300 rounded px-2 py-0.5 text-sm w-20 focus:outline-none focus:ring-2 focus:ring-black"
                                    />
                                    <button x-show="!editing" @click.stop="startEdit()" class="shrink-0 opacity-0 group-hover:opacity-40 hover:!opacity-100 transition-opacity" title="Edit unit">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a2 2 0 01-1.414.586H8v-2.414a2 2 0 01.586-1.414z"/></svg>
                                    </button>
                                </div>
                            </td>

                            {{-- Stdev (double-click to edit) --}}
                            <td wire:key="stdev-field-{{ $c->id }}" class="px-4 py-3">
                                <div
                                    x-data="{
                                        editing: false,
                                        value: @js($c->stdev !== null ? number_format((float)$c->stdev, 4, '.', '') : ''),
                                        original: @js($c->stdev !== null ? number_format((float)$c->stdev, 4, '.', '') : ''),
                                        cancelled: false,
                                        startEdit() { this.editing = true; this.$nextTick(() => this.$refs.stdevInput.select()); },
                                        cancel() { this.cancelled = true; this.value = this.original; this.editing = false; },
                                        save() {
                                            if (!this.cancelled && this.value !== this.original) {
                                                $wire.updateStdev({{ $c->id }}, this.value);
                                                this.original = this.value;
                                            }
                                            this.cancelled = false;
                                            this.editing = false;
                                ...
                                >
                                    <span x-show="!editing" @dblclick.stop="startEdit()" class="cursor-default" title="Double-click to edit" x-text="value || '—'"></span>
                                    <input
                                        x-show="editing" x-ref="stdevInput" x-model="value" type="text" inputmode="decimal"
                                        @click.stop @keydown.enter.prevent="save()" @keydown.escape.prevent="cancel()" @blur="save()"
                                        class="border border-gray-300 rounded px-2 py-0.5 text-sm w-24 focus:outline-none focus:ring-2 focus:ring-black"
                                    />
                                    <button x-show="!editing" @click.stop="startEdit()" class="shrink-0 opacity-0 group-hover:opacity-40 hover:!opacity-100 transition-opacity" title="Edit stdev">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a2 2 0 01-1.414.586H8v-2.414a2 2 0 01.586-1.414z"/></svg>
                                    </button>
                                </div>
                            </td>
        ...
        >
            ...
            <div class="border rounded-lg p-4 space-y-3">
                ...
                <p class="text-xs text-gray-500">
                    With headers, use columns like <code>name</code>, <code>compound</code>, <code>ri</code>, <code>mz</code>/<code>m/z</code>, plus <code>value</code>, <code>unit</code>, and <code>stdev</code> for this analysis run's measurements.
                    Without headers the first three columns are read as name, RI, and m/z (and a 4th/5th/6th column as value/unit/stdev).
                </p>
            </div>

            {{-- Manual entry --}}
            @php
                $manualRowShape = "{ name: '', ri: '', mz: '', value: '', unit: '', stdev: '' }";
                $manualGridCols = '1fr_1fr_1fr_1fr_1fr_1fr_auto';
            @endphp
            <div class="border rounded-lg p-4 space-y-3">
                <div
                    x-data="{
                        rows: [{{ $manualRowShape }}],
                        addRow()  { this.rows.push({{ $manualRowShape }}) },
                        removeRow(i) { this.rows.splice(i, 1) },
                        save() { $wire.saveManualCompounds(this.rows) }
                    }"
                    x-init="$watch('$wire.showImportModal', val => {
                        if (val) rows = [{{ $manualRowShape }}]
                    })"
                >
                    <div class="flex items-center justify-between mb-3">
                        <div>
                    ...
                    <div class="grid grid-cols-[{{ $manualGridCols }}] gap-2 text-xs font-medium text-gray-500 px-1 mb-1">
                    ...
                    <div wire:ignore>
                        <template x-for="(row, i) in rows" :key="i">
                            <div class="grid grid-cols-[{{ $manualGridCols }}] gap-2 items-center mb-2">
                                ...
                                <input x-model="row.value" placeholder="e.g. 12.3" class="border px-3 py-2 rounded text-sm">
                                <input x-model="row.unit"  placeholder="e.g. %"    class="border px-3 py-2 rounded text-sm">
                                <input x-model="row.stdev" placeholder="optional"  class="border px-3 py-2 rounded text-sm">
